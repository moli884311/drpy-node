...
const loadCustomData = async () => {
  ...
  try {
    ...
    customData.value = result.data || { sites: [], lives: [] }
  } catch (e) {
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
            编辑 {{ CUSTOM_JSON_PATH }}，管理自定义站点和直播源列表
          </p>
        </div>
        <button
    ...
    <div class="custom-source-content space-y-6 mt-6">
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ sitesCount }}</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">自定义站点</p>
        </div>
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-green-600">{{ livesCount }}</p>
          <p class="text-sm text-gray-500 dark:text-gray-400">直播源</p>
        </div>
      </div>
      ...
      <section>
        ...
        <div class="card p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer" @click="editJsonFile">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h4 class="font-medium text-gray-900 dark:text-gray-100">moli.json</h4>
      ...
      <section>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          <div
            ...
            :key="site.key"
            class="card p-4 hover:shadow-md transition-shadow border border-transparent hover:border-orange-200 dark:hover:border-orange-800"
          >
